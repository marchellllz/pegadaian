...
if (!isset($_SESSION['usr']) || !isset($_SESSION['role']) || $_SESSION['role'] !== 'supervisor' || $_SESSION['role'] !== 'supervisor') {
...
// filter cabang biar cabang lain nggak bisa ikut nimbrung
...
if ($filterTahun !== '') {
    $escapedStatus = $conn->real_escape_string($filterTahun);
...
// Header kolom (dengan Usia Denda)
$headers = [
    ...
    'Usia (Hari)', 'Usia Denda (Hari)', 'Total Tagihan', 'Sisa Tagihan', 'Status'
];
...
while ($row = $result->fetch_assoc()) {
    ...
    // Format sisa tagihan untuk Excel - gunakan angka murni atau 0
    $sisaTagihanValue = ($status === 'ditolak' || $status === 'pending') ? 0 : $sisaTagihan;
    ...
    $sheet->fromArray([
        ...
        $row['tanggal_masuk'],
        $row['tanggal_keluar'] ?? '-',
        $nilai,  // Tanpa number_format
        $nilaitaksir,
        $bunga,  // Tanpa number_format
        $biayaadm,
        $denda_total,
        $denda,  // Tanpa number_format
        $usia,
        $usiaDendaText,
        $total,  // Tanpa number_format
        ($status === 'ditolak' || $status === 'pending') ? 0 : $sisaTagihan,  // Angka murni atau 0
...
// Auto size kolom
foreach (range('A', 'R') as $c) { // Update sampai kolom O karena ada 15 kolom
...
// Output file
$filename = 'data_gadai_supervisor_' . date('Ymd_His') . '.xlsx';
